...
void main()
{
    ...
    // ambient
    vec3 ambient = (light.ambient + (light.color * light.ambient / 4)) * vec3(texture(material.diffuse, TexCoords));
    ...
    vec3 specular = light.specular * (((vec3(1.0, 1.0, 1.0) - light.color) / light.color_reflection) + light.color) * spec * vec3(texture(material.specular, TexCoords));
